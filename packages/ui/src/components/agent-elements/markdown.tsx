...
export function Markdown({ content, className }: MarkdownProps) {
  ...
  const components: Components = {
    ...
    // Imágenes en markdown (p. ej. las generadas por el asistente): tamaño acotado,
    // bordes redondeados y carga diferida.
    img: ({ alt, ...props }) => (
      <img
        alt={alt ?? ""}
        {...props}
        className="my-2 max-h-96 max-w-full rounded-an-message border border-an-border-color object-contain"
        loading="lazy"
