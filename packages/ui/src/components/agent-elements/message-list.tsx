...
function getLastAssistantHasContent(messages: UIMessage[]) {
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    return (msg.parts ?? []).some((part) => {
      ...
      if (isReasoningPart(part)) return part.text.trim().length > 0;
...
export const MessageList = memo(function MessageList({
  ...
  // ScrollShadow: difuminado en los bordes según haya contenido para desplazar.
  ...
  useLayoutEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      if (shouldAutoScrollRef.current) {
        // Pegado al fondo: cuando el contenido crece (p. ej. el markdown termina
        // de renderizar al abrir una conversación larga), mantenemos el scroll
        // abajo para que no se vea "estirándose" y aterrice en el último mensaje.
        container.scrollTop = container.scrollHeight;
      } else {
  ...
  // Recalcula el ScrollShadow cuando cambia el contenido (p. ej. llega un mensaje).
  useEffect(() => {
    updateScrollShadow();
  }, [updateScrollShadow, normalizedMessages]);
  ...
  // Mask del ScrollShadow: difumina solo el borde que tiene contenido oculto.
  const scrollMask = `linear-gradient(to bottom, ${
    scrollShadow.top ? "transparent" : "black"
  } 0, black 2rem, black calc(100% - 2rem), ${
    scrollShadow.bottom ? "transparent" : "black"
  } 100%)`;
  ...
  const { elements } = useMemo(() => {
    ...
    while (i < parts.length) {
      ...
      if (isReasoningPart(part)) {
        if (part.text.trim()) {
          const animating = isLast && isStreaming && part.state !== "done";
          elems.push(
            <ThinkingTool
              ...
              state={animating ? "animating" : "complete"}
              onComplete={() => {}}
              defaultOpen={false}
            />,
          );
