...
function getFileFromPart(part: unknown) {
  ...
  // Imágenes y archivos se muestran ambos como chip (nombre + peso). Las imágenes
  // conservan `url` para pintar la miniatura y abrir el lightbox.
  return {
    filename,
    size: filePart.size,
    isImage,
...
export const UserMessage = memo(function UserMessage({
  ...
  for (const part of message.parts ?? []) {
    // Los `type: "file"` (imágenes y archivos) se muestran como chip con nombre/peso.
    // Solo los `type: "image"`/`data-image` sueltos (sin metadatos) caen a thumbnail.
